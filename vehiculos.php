...
if (isset($_POST['guardar'])) {
	...
	$sql->Query("INSERT INTO vehiculos_attr  VALUES (
		NULL, 
		'".$vid."', 
		'".__($_POST['att_colorext'])."', 
		'".__($_POST['colorint'])."', 
		'".__($_POST['vestiduras'])."', 
		'".__($_POST['transmision'])."', 
		'".__($_POST['nummotor'])."', 
		'".__($_POST['numserie'])."', 
		'".__($_POST['att_placas'])."', 
		'".__($_POST['att_placasenti'])."', 
		'".__($_POST['kilometraje'])."', 
		'".__($_POST['att_tenencias'])."', 
		'".__($_POST['att_equipamiento'])."', 
		'".__($_POST['aire'])."', 
		'".__($_POST['stereo'])."', 
		'".__($_POST['cd'])."', 
		'".__($_POST['quemacocos'])."', 
		...
		'".__($_POST['att_precioventa'])."',
		'".__($_POST['att_preciooferta'])."',
		'".__($_POST['att_disponible'])."',
		'".__($_POST['att_web'])."'
		)");
}
...
if (isset($_POST['editar'])) {
	...
	$sql->Query("UPDATE  vehiculos_attr 
		SET att_colorext='".__($_POST['att_colorext'])."', 
		att_colorint = '".__($_POST['colorint'])."', 
		att_vestiduras='".__($_POST['vestiduras'])."', 
		att_transmision='".__($_POST['transmision'])."', 
		att_nummotor='".__($_POST['nummotor'])."', 
		att_numserie='".__($_POST['numserie'])."', 
		att_placas='".__($_POST['att_placas'])."', 
		att_placasenti='".__($_POST['att_placasenti'])."', 
		att_kilometraje='".__($_POST['kilometraje'])."', 
		att_tenencias='".__($_POST['att_tenencias'])."', 
		att_equipamiento='".__($_POST['att_equipamiento'])."', 
		att_aire='".__($_POST['aire'])."', 
		att_stereo='".__($_POST['stereo'])."', 
		att_cd='".__($_POST['cd'])."', 
		att_quemacocos='".__($_POST['quemacocos'])."', 
		att_rines='".__($_POST['rines'])."', 
		att_bolsasaire='".__($_POST['bolsasaire'])."', 
		att_cilindros='".__($_POST['cilindros'])."', 
		att_vidrios='".__($_POST['vidrios'])."', 
		att_seguros='".__($_POST['seguros'])."', 
		att_qty='".__($_POST['att_qty'])."', 
		att_anotaciones='".__($_POST['comentarios'])."', 
		att_fechafac='".__($_POST['fechafac'])."', 
		att_expedida='".__($_POST['expedida'])."', 
		att_folio='".__($_POST['folio'])."', 
		att_vu='".__($_POST['att_vu'])."', 
		att_fotos='".__($_POST['att_fotos'])."', 
		att_preciocompra='".__($_POST['att_preciocompra'])."',
		att_precioventa='".__($_POST['att_precioventa'])."',
		att_preciooferta='".__($_POST['att_preciooferta'])."', 
		att_disponible='".__($_POST['att_disponible'])."', 
		att_web='".__($_POST['att_web'])."'
		WHERE ve_id='".__($_POST['ve_id'])."' ");
...
<form class="col-xs-12" id="formatable" action="/admin.php?car" method="post">
	<table class="table table-striped">
		<tbody>
			...
			<tr>
				...
				<td>Equipamiento:</td>
				<td><label class="radito"> Si <input value="1" type="radio" name="att_equipamiento" <?php if($feve AND $att->att_equipamiento==1) echo 'checked';?> ></label><label class="radito"> No <input value="0" type="radio"  name="att_equipamiento" <?php if($feve AND $att->att_equipamiento==0) echo 'checked';?> ></label></td>
				<td>Cantidad de unidades:</td>
				<td> <input name="att_qty" type="number" class="form-control" value="<?php if($feve) echo $att->att_qty; ?>"></td>
			</tr>
			<tr>
				<td  colspan="6"><b>Equipamiento:</b></td>
			</tr>
			<tr>
				<td>A/C:</td>
				<td><label class="radito"> Si <input value="1" type="radio" name="aire"  <?php if($feve AND $att->att_aire==1) echo 'checked';?> ></label><label class="radito"> No <input value="0" type="radio"  name="aire" <?php if($feve AND $att->att_aire==0) echo 'checked';  ?>></label></td>
				<td>Estereo:</td>
				<td><label class="radito"> Si <input value="1" type="radio" name="stereo" <?php if($feve AND $att->att_stereo==1) echo 'checked';?>></label><label class="radito"> No <input value="0" type="radio"  name="stereo" <?php if($feve AND $att->att_stereo==0) echo 'checked';?>></label></td>
				<td>CD:</td>
				<td><label class="radito"> Si <input value="1" type="radio" name="cd" <?php if($feve AND $att->att_cd==1) echo 'checked';?> ></label><label class="radito"> No <input value="0" type="radio"  name="cd" <?php if($feve AND $att->att_cd==0) echo 'checked';?> ></label></td>
			</tr>
			<tr>
				<td>Quemacocos:</td>
				<td><label class="radito"> Si <input value="1" type="radio" name="quemacocos" <?php if($feve AND $att->att_quemacocos==1) echo 'checked';?> ></label><label class="radito"> No <input value="0" type="radio"  name="quemacocos" <?php if($feve AND $att->att_quemacocos==0) echo 'checked';?> ></label></td>
				<td>Rines:</td>
				<td><label class="radito"> Si <input value="1" type="radio" name="rines" <?php if($feve AND $att->att_rines==1) echo 'checked';?> ></label><label class="radito"> No <input value="0" type="radio"  name="rines" <?php if($feve AND $att->att_rines==0) echo 'checked';?>></label></td>
				<td>Bolsas de aire:</td>
				<td><label class="radito"> Si <input value="1" type="radio" name="bolsasaire" <?php if($feve AND $att->att_bolsasaire==1) echo 'checked';?> ></label><label class="radito"> No <input value="0" type="radio"  name="bolsasaire" <?php if($feve AND $att->att_bolsasaire==0) echo 'checked';?>></label></td>
			</tr>
			<tr>
				<td>Comentarios:</td>
			...
			<tr>
				<td colspan="6"><b>Facturación</b></td>
			</tr>
			<tr>
				<td>Fecha (fac):</td>
			...
			<tr>
				<td colspan="6"><b>Fotografias</b></td>
			</tr>
			<tr>
			...
			<tr>
				...
				<td>
					<label class="radito"> Si <input value="1" type="radio" name="att_disponible" <?php if($feve AND $att->att_disponible==1) echo 'checked';?>></label><label class="radito"> No <input value="0" type="radio"  name="att_disponible" <?php if($feve AND $att->att_disponible==0) echo 'checked';?>></label>
				</td>
				<td>Web</td>
				<td>
					<label class="radito"> Si <input value="1" type="radio" name="att_web" <?php if($feve AND $att->att_web==1) echo 'checked';?> ></label><label class="radito"> No <input value="0" type="radio"  name="att_web" <?php if($feve AND $att->att_web==0) echo 'checked';?>></label>
				</td>
			</tr>
			
			<tr>
				<td colspan="6" class="text-center"><input value="<?php echo $titulo;?>" name="<?php echo $name;?>"  type="submit"   class="btn-lg btn btn-success"></td>
			</tr>
		</tbody>
	</table>
</form>
			...
			<?php
				if ($usu->num_rows>0) {
					while ($u=$usu->fetch_object()) {
						...
						echo "<tr> <td>".$u->ve_id."</td> <td>".$u->ve_fechaad."</td> <td>".$mark."</td> <td>".$u->ve_modelo."</td>   <td> <a href='/admin.php?car&c=".$u->ve_id."' class='btn btn-warning'>Editar</a> <a href='/admin.php?car&e=".$u->ve_id."' class='btn btn-danger'>Eliminar</a></td> </tr>";
					}
				}
			?>
		</table>
