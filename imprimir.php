...
$att_colorext = $sql->Query("SELECT * FROM colores WHERE co_id='".$_SESSION['compra']['att_colorext']."' ");
if ($att_colorext->num_rows>0) {
	$att_colorext = $att_colorext->fetch_object();
}

$att_colorint = $sql->Query("SELECT * FROM colores WHERE co_id='".$_SESSION['compra']['att_colorint']."' ");
if ($att_colorint->num_rows>0) {
	$att_colorint = $att_colorint->fetch_object();
}		
?>
<!DOCTYPE html>
<html lang="es">
