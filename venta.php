...
if (isset($_POST['guardar'])) {
	...
	// insertar venta
	$compra=$sql->Query("INSERT INTO ventas (ven_id,ve_id,ve_vu,usu_id,suc_id,cli_id,ven_fecha,av_id) VALUES (NULL,'".__($_POST['ve_id'])."','".__($_POST['att_vu'])."','".__($_POST['usu_id'])."','".__($_POST['suc_id'])."','".$cli_id."','".__($_POST['ven_fecha'])."','".$av_id."')");
	$ven_id=$sql->insert_id;

	// insertar presupuesto
	
	$pre_fpe= implode($_POST['pre_fpe'], ',');
	//$pre_fpe=__($_POST['pre_fpe1']); // hacer por cada input una fecha separada por coma  ENGANCHES
	$pre_fpm=$_POST['pre_fpm1']; // asi se queda solo una fecha de cada mes   MENSUALIDADES
	
	$pre_fpa= implode($_POST['pre_fpa'], ',');
	//$pre_fpa=__($_POST['pre_fpa']); // hacer por cada input una fecha separada por coma  ANUALIDADES
...
<form class="col-xs-12" id="formatable" action="?venta" method="post" name="formatable">
	<table class="table table-striped">
		...
		<tr>
			<td><b>Cliente</b></td>
			<td>
				<select id="cli_id" class="form-control" name="cli_id" onchange="checar(this.value)">
		...
		<tr class="clie">
			<td>Nombre(s):</td>
			<td colspan="2"><input id="cli_nombre" name='cli_nombre' type="text" class="form-control"></td>
			<td>Apellido(s):</td>
			<td colspan="2"><input id="cli_apellido" name='cli_apellido' type="text" class="form-control"></td>
		</tr>
		<tr class="prove">
		...
		<tr class="clie">
			<td>Calle:</td>
			<td><input id="cli_dir1" name="cli_dir1" type="text" class="form-control"></td>
			<td>Número:</td>
			<td><input id="cli_dir2" name="cli_dir2" type="text" class="form-control"></td>
			<td>Colonia:</td>
			<td><input id="cli_dir3" name="cli_dir3" type="text" class="form-control"></td>
		</tr>
		<tr class="clie">
			<td>Ciudad:</td>
			<td><input id="cli_ciudad" name="cli_ciudad" type="text" class="form-control"></td>
			<td>Estado:</td>
			<td><input id="cli_estado" name="cli_estado" type="text" class="form-control"></td>
			<td>CP:</td>
		...
		<tr class="clie">
			<td>Nombre(s):</td>
			<td colspan="2"><input id="av_nombre" name='av_nombre' type="text" class="form-control"></td>
			<td>Apellido(s):</td>
			<td colspan="2"><input id="av_apellidos" name='av_apellidos' type="text" class="form-control"></td>
		</tr>
		<tr class="prove">
			<td colspan="6">Dirección:</td>
		</tr>
		...
		<tr>
			...
			<td>Parentezco:</td>
			<td><input id="av_parentezco" name="av_parentezco" type="text" class="form-control"></td>
			<td>RFC:</td>
		...
		<tr>
			...
			<td>
				<select id="ve_id" class="form-control" name="ve_id" onchange="unidad(this.value)">
					<option value="seleccione" >Seleccione</option>
					<?php
		...
		<tr>
			<td colspan="6"><b>Presupuesto</b></td>
		</tr>
		<tr>
			<td>Precio Venta</td>
			<td colspan="2"><input onkeyup="costototal();" name="pre_precio" type="number" class="form-control"> </td>
